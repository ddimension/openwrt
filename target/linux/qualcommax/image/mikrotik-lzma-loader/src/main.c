/*
 * LZMA-Loader fuer MikroTik Chateau 5G R17 ax — Neuaufbau.
 *
 * Bewusst OHNE serielle Ausgabe. Die UART hat sich als Diagnosemittel als
 * unzuverlaessig erwiesen: die Ausgabe endete reproduzierbar an derselben
 * Stelle, obwohl derselbe Speicherzugriff wenige Instruktionen zuvor als Sonde
 * nachweislich funktionierte. Auf dieser Grundlage sind mehrere Fehldiagnosen
 * entstanden. Signalisiert wird stattdessen ueber die System-LED — ein einzelner
 * Registerschreibzugriff ohne FIFO und ohne Zustandsmaschine.
 *
 * Blinkcodes (gruene System-LED, GPIO 20):
 *   1x  Loader laeuft, Payload-Header wird gelesen
 *   2x  Header gueltig, Entpacken beginnt
 *   3x  Entpacken erfolgreich
 *   Dauerlicht  Fehler (siehe blink_forever)
 *
 * Speicherplan (loader.lds):
 *   0x01100000  Loader (Code, Daten, BSS, Stack unterhalb des Codes)
 *   0x42000000  LZMA-Payload   — RouterBOOT-erprobte Ladeadresse
 *   0x44000000  Entpackziel    — schreibt nur unser Code
 *   0x48000000  DTB            — RouterBOOT-erprobte Ladeadresse
 */
...
/* TLMM: GPIO_CFG(n) = 0x01000000 + n*0x1000 (Bit 9 = Output Enable),
 *       GPIO_IN_OUT(n) = +4 (Bit 1 = Ausgabewert). GPIO 20 = system-green. */
#define TLMM_BASE	0x01000000UL
#define LED_GPIO	20
#define LED_CFG		(TLMM_BASE + LED_GPIO * 0x1000)
#define LED_IO		(LED_CFG + 4)

/* Probs-Tabelle fuer lc+lp <= 4; "lzma e" nutzt lc=3/lp=0. */
#define MAX_LC_LP	4
...
/* ---- UARTDM (blsp1_uart3 = Konsole laut stdout-path) ----
 * RouterBOOT hat die UART initialisiert, daher kein Clock-/Baudraten-Setup.
 * Protokoll 1:1 aus drivers/tty/serial/msm_serial.c (__msm_console_write,
 * is_uartdm): NCF_TX = Zeichenzahl, danach je 4 gepackte Bytes nach TF,
 * Handshake ueber SR.TX_READY.
 */
...
#define UART_TF			0x70
#define UART_TIMEOUT		0x20000		/* nie endlos warten */

/* ---- Watchdog (qcom,kpss-wdt), Offsets aus drivers/watchdog/qcom-wdt.c ---- */
...
/*
 * Watchdog: Ausgangszustand sichern und fuers Entpacken abschalten. Vor dem
 * Kernel-Sprung wird er zurueckgesetzt (frisch getriggert) und wieder in den
 * urspruenglichen Zustand versetzt — ein haengender Kernel-Boot erholt sich
 * dann wieder von selbst, so wie RouterBOOT es vorgesehen hat.
 */
...
{
	wr32(WDT_BASE + WDT_RST, 1);		/* Zaehler neu starten */
